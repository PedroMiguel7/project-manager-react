        ...
        return (
            <>
                ...
                <div ref={dropRef} className="scroll d-flex flex-column align-items-center" style={{ overflowY:'auto', height: "745px" }}>
                    {
                        props.tarefas.map(p => (
                            <MostrarTarefasCard tarefa={p} statusPROJETO={props.statusPROJETO} status={props.status} key={p.id_task} equipe_id={props.equipe_id} prioridade={p.prioridade} id_task={p.id_task} descricao_task={p.descricao_task} nome_pessoa={p.nome_pessoa} />
                        ))
                    }
                </div>
            </>
